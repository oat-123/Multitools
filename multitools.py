...
if mode == "night_duty":
    ...
    if st.button("🚀 จัดยอดและส่งออกไฟล์"):
        ...
        # รวมคอลัมน์ 'ยศ', 'ชื่อ', 'สกุล' เป็นคอลัมน์ 'ยศ ชื่อ-สกุล'
        df["ยศ ชื่อ-สกุล"] = df["ยศ"] + " " + df["ชื่อ"] + " " + df["สกุล"]
        
        # ลบคอลัมน์เดิม 'ยศ', 'ชื่อ', 'สกุล' ออก
        df = df.drop(columns=["ยศ", "ชื่อ", "สกุล"])
        
        # จัดลำดับคอลัมน์ใหม่ (รวม "ยศ ชื่อ-สกุล")
        columns_order = ["ลำดับ", "ยศ ชื่อ-สกุล", "ชั้นปีที่", "ตอน", "ตำแหน่ง", "สังกัด", "หมายเหตุ"]
        df = df[columns_order]
