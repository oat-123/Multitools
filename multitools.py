...
# โค้ดเวอร์ชันปรับปรุงแล้ว สำหรับฟีเจอร์ "จัดยอดพิธี"
    ...
    # -- ตั้งค่าเริ่มต้นให้ session_state ถ้ายังไม่มี
    if "hide_duty_selector" not in st.session_state:
        st.session_state.hide_duty_selector = False
    
    if "hide_club_selector" not in st.session_state:
        st.session_state.hide_club_selector = False
    
    # -- รายการตัวเลือก
    ตัวเลือก_หน้าที่ = ["ชั้นกรม", "ชั้นพัน", "ฝอ.1", "ฝอ.4", "ฝอ.5"]
    ตัวเลือก_ชมรม = ["กรีฑา", "จักรยาน", "ไซเบอร์", "ดนตรีไทย", "ดนตรีสากล", 
                    "ดาบสากล", "นิเทศ", "บาส", "โปโลน้ำ", "ฟุตบอล", "ยูโด", "รักบี้", "แบตมินตัน"]
    
    # ===== หน้าที่ (ซ่อน multiselect เมื่อครบ) =====
    if not st.session_state.hide_duty_selector:
        ตัวกรอง_หน้าที่_เลือก = st.multiselect("⛔ไม่เลือกคนที่มีหน้าที่", ["เลือกทั้งหมด"] + ตัวเลือก_หน้าที่, key="duty_select")
        
        if ("เลือกทั้งหมด" in st.session_state.duty_select) or (set(st.session_state.duty_select) >= set(ตัวเลือก_หน้าที่)):
            st.session_state.hide_duty_selector = True
            ตัวกรอง_หน้าที่ = ตัวเลือก_หน้าที่
            st.info("✅ เลือกไม่เอาทุกหน้าที่แล้ว")
        else:
            ตัวกรอง_หน้าที่ = st.session_state.duty_select
    else:
        st.info("✅ เลือกไม่เอาทุกหน้าที่แล้ว")
    
    # ===== ชมรม (ซ่อน multiselect เมื่อครบ) =====
    if not st.session_state.hide_club_selector:
        excluded_clubs = st.multiselect("⛔ไม่เลือกชมรม", ["เลือกทั้งหมด"] + ตัวเลือก_ชมรม, key="club_select")
        
        if ("เลือกทั้งหมด" in st.session_state.club_select) or (set(st.session_state.club_select) >= set(ตัวเลือก_ชมรม)):
            st.session_state.hide_club_selector = True
            filtered_clubs = ตัวเลือก_ชมรม
            st.info("✅ เลือกไม่เอาทุกชมรมแล้ว")
        else:
            filtered_clubs = st.session_state.club_select
    else:
        st.info("✅ เลือกไม่เอาทุกชมรมแล้ว")
