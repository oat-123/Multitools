...
if mode == "night_duty":
    ...
    # 2. อัปโหลดไฟล์ยอด
    ยอด_file = st.file_uploader("อัปโหลดไฟล์รายชื่อผู้โดนยอด (.xlsx)", type="xlsx")
    ...
    if ยอด_file:
        # อ่านไฟล์ยอด
        ยอด_df = pd.read_excel(ยอด_file)

        # ล้างช่องว่างจากชื่อคอลัมน์
        ยอด_df.columns = ยอด_df.columns.str.strip()
        ชั้น4พัน4.columns = ชั้น4พัน4.columns.str.strip()
    
        st.write("🧾 คอลัมน์ในไฟล์ยอด:", ยอด_df.columns.tolist())  # แสดงให้เช็ค
        
        # สร้างคอลัมน์ชื่อเต็ม
        ยอด_df["ชื่อเต็ม"] = ยอด_df["ยศ ชื่อ-สกุล"].astype(str).str.strip()
        ชั้น4พัน4["ชื่อเต็ม"] = ชั้น4พัน4["ยศ ชื่อ-สกุล"].astype(str).str.strip()
    
        # ถ้ายังไม่มีคอลัมน์ "สถิติโดนยอด" ให้สร้าง
        ...
        st.download_button("📥 ดาวน์โหลดไฟล์ชั้น4พัน4 ที่อัปเดตแล้ว", output.getvalue(), file_name="ชั้น4พัน4_อัปเดต.xlsx")
