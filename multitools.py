...
if mode == "night_duty":
    ...
    if "ยศ ชื่อ-สกุล" in ยอด_df.columns:
        # แยกด้วยช่องว่าง
        split_cols = ยอด_df["ยศ ชื่อ-สกุล"].astype(str).str.split(" ", n=2, expand=True)
        split_cols.columns = ["ยศ", "ชื่อ", "สกุล"]

        # รวมเข้ากับ dataframe เดิม
        ยอด_df = pd.concat([ยอด_df, split_cols], axis=1)

    # สร้างชื่อเต็มทั้งสองฝั่ง
