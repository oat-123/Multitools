...
if mode == "night_duty":
    ...
    # (แสดงชื่อคอลัมน์ไว้ debug)
    st.write("🧾 คอลัมน์ที่พบ:", ชั้น4พัน4.columns.tolist())
    
    # สร้างคอลัมน์ "ยศ ชื่อ-สกุล"
    ชั้น4พัน4["ยศ ชื่อ-สกุล"] = ชั้น4พัน4["ยศ"].astype(str).str.strip() + " " + \
                                ชั้น4พัน4["ชื่อ"].astype(str).str.strip() + " " + \
                                ชั้น4พัน4["สกุล"].astype(str).str.strip()
    ...
    if ยอด_file:
        ...
        # แยกชื่อ-นามสกุลจากคอลัมน์รวม
        ยอด_df = split_fullname(ยอด_df)
    
        # สร้างคอลัมน์เทียบ
        ชั้น4พัน4["ชื่อเต็ม"] = ชั้น4พัน4["ยศ"].astype(str).str.strip() + " " + ชั้น4พัน4["ชื่อ"].astype(str).str.strip() + " " + ชั้น4พัน4["สกุล"].astype(str).str.strip()
        ยอด_df["ชื่อเต็ม"] = ยอด_df["ยศ"].astype(str).str.strip() + " " + ยอด_df["ชื่อ"].astype(str).str.strip() + " " + ยอด_df["สกุล"].astype(str).str.strip()
    
        # เลือกระดับความเหนื่อย
        เหนื่อย = st.slider("ระดับความเหนื่อยของยอดนี้ (1–5)", 1, 5, 3)
    
        # อัปเดตคอลัมน์คะแนน
        def check_update(row):
            if row["ชื่อเต็ม"] in ยอด_df["ชื่อเต็ม"].values:
                return row.get("สถิติโดนยอด", 0) + เหนื่อย
            return row.get("สถิติโดนยอด", 0)
