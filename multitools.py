...
if mode == "night_duty":
    ...
    # 3. สร้างคอลัมน์ "ยศ ชื่อ-สกุล" ในชั้น4พัน4 และยอด_df
    ชั้น4พัน4["ยศ ชื่อ-สกุล"] = ชั้น4พัน4["ยศ"].astype(str).str.strip() + " " + ชั้น4พัน4["ชื่อ"].astype(str).str.strip() + " " + ชั้น4พัน4["สกุล"].astype(str).str.strip()
    ...
    if ยอด_file:
        ...
        # สร้างคอลัมน์ "ยศ ชื่อ-สกุล" ในยอด_df
        ยอด_df["ยศ ชื่อ-สกุล"] = ยอด_df["ยศ"].astype(str).str.strip() + " " + ยอด_df["ชื่อ"].astype(str).str.strip() + " " + ยอด_df["สกุล"].astype(str).str.strip()

        def check_update(row):
            # ตรวจสอบว่าชื่อเต็มในชั้น4พัน4 ตรงกับชื่อเต็มในยอด_df หรือไม่
            if row["ยศ ชื่อ-สกุล"] in ยอด_df["ยศ ชื่อ-สกุล"].values:
        ...
        output.seek(0)  # รีเซ็ตตำแหน่งของ pointer ไปที่จุดเริ่มต้น
        st.download_button("📥 ดาวน์โหลดไฟล์อัปเดต", output, file_name="ชั้น4พัน4_อัปเดต.xlsx")
        
        st.success("อัปเดตเรียบร้อย ✅")
    else:
        st.error("กรุณาอัปโหลดไฟล์ยอดก่อน")
