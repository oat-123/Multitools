...
if mode == "night_duty":
    ...
    if ยอด_file:
        # 3. โหลดไฟล์ยอด โดยข้ามหัวตารางที่ผสาน
        ยอด_df = pd.read_excel(ยอด_file, header=None, skiprows=3)  # 👈 ข้าม B1–B3
        ยอด_df = ยอด_df.dropna(how='all')  # ลบแถวว่าง

        # โหลดไฟล์หลัก
        ชั้น4พัน4 = pd.read_excel("ชั้น4พัน4.xlsx", header=None, skiprows=3)
        ชั้น4พัน4 = ชั้น4พัน4.dropna(how='all')

        # 4. สร้างชื่อเต็มจาก คอลัมน์ C (index 2) + D (index 3)
        ยอด_df["ชื่อเต็ม"] = ยอด_df.iloc[:, 2].astype(str).str.strip() + " " + ยอด_df.iloc[:, 3].astype(str).str.strip()
        ชั้น4พัน4["ชื่อเต็ม"] = ชั้น4พัน4.iloc[:, 2].astype(str).str.strip() + " " + ชั้น4พัน4.iloc[:, 3].astype(str).str.strip()

        # 5. สร้างคอลัมน์เก็บสถิติถ้ายังไม่มี
        if "สถิติโดนยอด" not in ชั้น4พัน4.columns:
            ชั้น4พัน4["สถิติโดนยอด"] = 0
        ...
        ชั้น4พัน4["สถิติโดนยอด"] = ชั้น4พัน4.apply(update_score, axis=1)

        # 7. ดาวน์โหลดไฟล์ใหม่
        from io import BytesIO
        output = BytesIO()
        ชั้น4พัน4.to_excel(output, index=False, header=False)  # ไม่ใส่ header เพราะเรา skip ไปแล้ว
        st.download_button("📥 ดาวน์โหลดไฟล์อัปเดต", output.getvalue(), file_name="ชั้น4พัน4_อัปเดต.xlsx")
        st.success("✅ อัปเดตสถิติโดยใช้ข้อมูลจาก C D สำเร็จแล้ว")
