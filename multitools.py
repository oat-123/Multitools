...
if mode == "night_duty":
    ...
    st.markdown("## 📊 สถิติการโดนยอด")
    st.markdown("กรุณาเลือกระดับความเหนื่อยและอัปโหลดไฟล์ยอดจากระบบ")

    # ระดับความเหนื่อย
    เหนื่อย = st.slider("🪫 ระดับความเหนื่อยของยอดนี้", 1, 5, 3, help="ระดับ 1 = สบาย ๆ, 5 = เหนื่อยสุดๆ")

    # อัปโหลดไฟล์
    ยอด_file = st.file_uploader("📤 อัปโหลดไฟล์ยอด (.xlsx)", type="xlsx", help="ข้ามหัวตาราง 3 แถว")

    if ยอด_file:
    st.success("📄 อัปโหลดไฟล์สำเร็จ! กรุณาเลือกระดับความเหนื่อย แล้วกดปุ่มด้านล่างเพื่ออัปเดตแต้ม")
    
    # --- STEP 2: ป้อนระดับความเหนื่อย
    เหนื่อย = st.slider("🪫 ระดับความเหนื่อย", 1, 5, 3)

    # --- STEP 3: ปุ่มอัปเดตแต้ม
    if st.button("✅ อัปเดตแต้ม"):
        # 1. โหลดไฟล์ยอด
        ...
        # 4. ตรวจสอบ/เพิ่มคอลัมน์ 'สถิติโดนยอด'
        ...
        st.success("✅ อัปเดต 'สถิติโดนยอด' สำเร็จเรียบร้อยแล้ว!")
        st.markdown("👉 [ดู Google Sheets](https://docs.google.com/spreadsheets/d/e/2PACX-1vSf6OB3YE98NPUBjuN7c7tdp93kmj0kEAQMvMiu4FECY4OgbQgQ-AWwz31TcabtrlzWPgcilDmsG4uZ/pubhtml)")
