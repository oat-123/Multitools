...
if mode == "night_duty":
    ...
    if ยอด_file:
        # 3. โหลดไฟล์ยอดที่อัปโหลด (ข้ามหัวตาราง 3 แถว)
        ...
        # 4. โหลดข้อมูลจาก Google Sheet
        ws = connect_gsheet()
        gsheet_data = ws.get_all_values()
        gsheet_df = pd.DataFrame(gsheet_data)
        gsheet_df.columns = gsheet_df.iloc[2]  # หัวตารางจริงอยู่แถว 3
        ...
        # 5. เพิ่มคอลัมน์ "สถิติโดนยอด" ถ้ายังไม่มี
        ...
        # 7. เขียนกลับไปที่ Google Sheets
        gsheet_df.drop(columns=["ชื่อเต็ม"], inplace=True)
        ws.update([gsheet_df.columns.tolist()] + gsheet_df.values.tolist())
