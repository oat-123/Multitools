...
if mode == "night_duty":
    ...
    if st.button("🚀 จัดยอดและส่งออกไฟล์"):
        ...
        # ตรวจสอบคอลัมน์ที่ต้องการรวม
        if "ยศ" in df.columns and "ชื่อ" in df.columns and "สกุล" in df.columns:
            # ถ้ามีคอลัมน์เหล่านี้อยู่, ให้ทำการรวม
            df["ยศ ชื่อ-สกุล"] = df["ยศ"] + " " + df["ชื่อ"] + " " + df["สกุล"]
            df = df.drop(columns=["ยศ", "ชื่อ", "สกุล"])  # ลบคอลัมน์เดิมออก
        
            # จัดลำดับคอลัมน์ใหม่ (ถ้าต้องการ)
            column_order = ["ลำดับ", "ยศ ชื่อ-สกุล", "ชั้นปีที่", "ตอน", "ตำแหน่ง", "สังกัด", "หมายเหตุ"]
            df = df[column_order]
        else:
            st.error("คอลัมน์ 'ยศ', 'ชื่อ', 'สกุล' ไม่พบใน DataFrame")
